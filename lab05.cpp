...
bool read_word_list() {
    ...
    fin.close();
    // printString(wordlist, wordCount);
    return true;
}
...
bool read_encrypted_messages(string encrypted[], int &message_len) {
    ...
    while (fin >> tempWords) 
    {
        encrypted[message_len] = tempWords;
        message_len += 1;
    }
    fin.close();
    // printString(encrypted, message_len);
    return true;
    // CODES ENDS HERE
}
...
bool find_valid_shift(string encrypted[], int message_len, string decrypted[]) {
    ...
    for(int i = 0; i < 26; ++i) 
    {
        for(int j = 0; j < message_len; ++j) 
        {
            ...
            for(int m = 0; m < decrypted[j].length(); ++m) 
            {
                decrypted[j][m] = (decrypted[j][m] - 'a' + i) % 26 + 'a';
            }
        }
        bool isValid = is_all_in_list(decrypted, message_len);
        for(int i = 0; i < message_len; ++i)
        {
            cout << decrypted[i] << " ";
        }
        cout << endl;
        cout << "Shift " << i << ": " << isValid << endl;
